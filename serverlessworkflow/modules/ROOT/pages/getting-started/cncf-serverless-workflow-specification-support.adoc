...
* link:{spec_doc_url}#correlation-definition[Correlation] has limited support, that evaluate correlation rules matching just a single event consumed per time with a workflow instance, the correlation among `N` different events to be consumed and matched with a workflow instance is not supported yet in Kogito. See xref:eventing/event-correlation-with-workflows.adoc[Event correlation with Serverless Workflows].
